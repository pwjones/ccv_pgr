...
gpu_error_t gpu_sub( gpu_context_t *ctx, unsigned char *staticBg)
{
	...
	char *errstr;
	unsigned char *temp;
	int siz = ctx->height * ctx->width;
	cudaMalloc( (void **)&temp, siz);
	cudaMemcpy( temp, staticBg, siz, cudaMemcpyHostToDevice);
	...
	if( cudaSuccess != cudaMemcpy( ctx->output_buffer_1, ctx->gpu_buffer_1, siz, cudaMemcpyDeviceToHost))
		//errstr = gpu_error();
		error = GPU_ERR_MEM;
	cudaFree(temp);
